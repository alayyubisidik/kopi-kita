...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    <!-- Page header -->
    <div class="mb-8">
        <h1 class="text-2xl md:text-3xl text-gray-800 font-bold">Add Product</h1>
    </div>
        ...
        <div class="p-6">
            <form action="{{ route('dashboard.products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input id="name" name="name" type="text" class="form-input w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('name') }}" required />
                        ...
                        <div>
                            ...
                            <input id="price" name="price" type="number" min="0" step="1" class="form-input w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('price') }}" required />
                            @error('price') <div class="text-xs text-red-500 mt-1">{{ $message }}</div> @enderror
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1" for="description">Description</label>
                            <textarea id="description" name="description" rows="4" class="form-textarea w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description') }}</textarea>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center mt-2">
                            <input id="is_available" name="is_available" type="checkbox" value="1" class="form-checkbox h-4 w-4 text-indigo-600 transition duration-150 ease-in-out" {{ old('is_available', true) ? 'checked' : '' }} />
                            <label for="is_available" class="ml-2 block text-sm leading-5 text-gray-900">Available (Can be ordered)</label>
                        </div>
                        
                        <div class="flex items-center mt-2">
                            <input id="is_customizable" name="is_customizable" type="checkbox" value="1" class="form-checkbox h-4 w-4 text-indigo-600 transition duration-150 ease-in-out" {{ old('is_customizable') ? 'checked' : '' }} onchange="toggleCustomization()" />
                        ...
                        <div id="customization_section" class="mt-4 p-4 bg-gray-50 rounded-md border border-gray-200 {{ old('is_customizable') ? '' : 'hidden' }}">
                                    ...
                                    @foreach($optionGroups as $group)
                                        <div class="flex items-center justify-between p-2 hover:bg-gray-100 rounded">
                                            <div class="flex items-center">
                                                <input id="group_{{ $group->id }}" name="option_groups[]" type="checkbox" value="{{ $group->id }}" class="form-checkbox h-4 w-4 text-indigo-600" {{ in_array($group->id, old('option_groups', [])) ? 'checked' : '' }} />
                                            ...
                                            <!-- Sort order will be handled by javascript or default to selection order -->
                                        </div>
                                    @endforeach
                                </div>
                ...
                <div class="mt-8 pt-5 border-t border-gray-200 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 bg-indigo-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-indigo-700">Save Product</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function toggleCustomization() {
        ...
        if (checkbox.checked) {
            ...
            // Uncheck all option groups when disabled
            const groupCheckboxes = section.querySelectorAll('input[type="checkbox"]');
