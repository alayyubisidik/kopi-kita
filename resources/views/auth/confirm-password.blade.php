...
<head>
    ...
    <title>Konfirmasi Password – Kopi Kita</title>
    <meta name="description" content="Konfirmasi password untuk melanjutkan ke area aman Kopi Kita.">
...
<body class="bg-surface text-on-surface font-sans text-body-md flex flex-col min-h-screen antialiased">
                ...
                <div class="w-full bg-surface-container-lowest rounded-xl p-space-lg shadow-xl shadow-primary-container/10 flex flex-col gap-space-lg relative overflow-hidden z-10">
                    ...
                    <div class="flex flex-col items-center text-center pt-space-xs">
                        <div class="relative flex items-center justify-center w-16 h-16 rounded-full bg-surface-container mb-space-sm shadow-md shadow-primary-container/10">
                            ...
                            <span class="material-symbols-outlined text-primary" style="font-size:32px;">shield_lock</span>
                        </div>
                        <h1 class="text-headline-lg font-headline-lg text-primary tracking-tight">Konfirmasi Password</h1>
                    ...
                    {{-- Form --}}
                    <form id="confirmForm" method="POST" action="{{ route('password.confirm') }}" class="flex flex-col gap-space-md">
                        @csrf

                        {{-- Password --}}
                        <div class="flex flex-col gap-1.5">
                            <label for="password" class="text-label-lg font-label-lg text-on-surface">Password</label>
                            <div class="relative flex items-center">
                                ...
                                <button type="button" onclick="togglePasswordVisibility()" aria-label="Toggle password"
    ...
    <script>
        ...
        document.getElementById('confirmForm').addEventListener('submit', function () {
            ...
            if (!btn || !text) return;
            btn.disabled = true;
            text.innerHTML = '<span class="material-symbols-outlined spin" style="font-size:20px;">sync</span> Memverifikasi...';
        });
    </script>
